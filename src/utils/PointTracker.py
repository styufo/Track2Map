...
class PointTracker(object):
    ...
    def init_tracking_points(self, c2w, depth_override=None):
        ...
        depth_for_init = render_pkg['depth']
        if depth_override is not None:
            depth_for_init = depth_override
            if depth_for_init.ndim == 3:
                depth_for_init = depth_for_init.squeeze(0)
            depth_for_init = depth_for_init.to(render_pkg['depth'].device).to(render_pkg['depth'].dtype)
        # project to 3D
        pts_3d = reproject(pts.long(), depth_for_init, fx, fy, cx, cy, c2w)
    ...
    def eval(self, c2w, idx):
        ...
        render_pkg = render(self.camera, self.net, self.background)
        # project to 3D
        pts_3d_gt = reproject(pts_gt.long(), render_pkg['depth'], fx, fy, cx, cy, c2w)/self.scale
    ...
    def get_2d_pts(self, c2w, camera=None, detach_scene=False, clamp_to_image=True):
