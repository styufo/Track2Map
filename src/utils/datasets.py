...
class StereoMIS(Dataset):
    def __init__(self, cfg, args, scale):
        ...
        if hasattr(args, 'pose_file') and args.pose_file is not None:
            ...
            pose_path = os.path.join(self.input_folder, pose_filename)
        if pose_path is not None and os.path.isfile(pose_path):
            self.load_poses(pose_path, frame_slice)
        else:
            warnings.warn(f"Pose file not found: {pose_path}. Falling back to identity poses.")
