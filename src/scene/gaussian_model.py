...
from src.scene.deformation import ExplicitDeformation, ExplicitSparseDeformation, ExplicitSparseFDMDeformation
...
class GaussianModel(nn.Module):
    def __init__(self, cfg, n_classes=7):
        ...
        deform_cfg = cfg["deform_network"]
        anchor_sampling = deform_cfg.get("anchor_sampling", "random")
        anchor_grid_cell_scale = deform_cfg.get("anchor_grid_cell_scale", 1.0)
        if deform_cfg['model'] == 'sparse':
            self._deformation = ExplicitSparseDeformation(
                subsample=deform_cfg['subsample'],
                anchor_sampling=anchor_sampling,
                anchor_grid_cell_scale=anchor_grid_cell_scale,
            )
        elif deform_cfg['model'] == 'sparse_fdm_lite':
            fdm_cfg = deform_cfg.get('fdm_lite', {})
            self._deformation = ExplicitSparseFDMDeformation(
                subsample=deform_cfg['subsample'],
                basis_num=fdm_cfg.get('basis_num', 9),
                basis_sigma=fdm_cfg.get('basis_sigma', 0.18),
                normalize_basis=fdm_cfg.get('normalize_basis', True),
                anchor_sampling=anchor_sampling,
                anchor_grid_cell_scale=anchor_grid_cell_scale,
            )
        elif deform_cfg['model'] == 'dense':
    ...
    def set_deformation_time(self, time: float):
        self.current_time = float(time)
        if hasattr(self._deformation, "set_time"):
            self._deformation.set_time(self.current_time)

    def forward(self, deform=True, time=None):
        """
            apply deformation and return 3D Gaussians for rasterization
        """
        if time is not None:
            self.set_deformation_time(time)
        elif hasattr(self._deformation, "set_time"):
            self._deformation.set_time(self.current_time)
        if deform:
            xyz, scales, rots = self._deformation(
                self._xyz, self._scaling, self._rotation, init=self.training, time=self.current_time
            )
        else:
    ...
    def create_from_pcd(self, rgb, depth, c2w, camera, tool_mask=None, spatial_lr_scale : float=1.0, downsample: int=2, semantics=None):
        with torch.no_grad():
            ...
            if semantics is None:
                semantics = torch.ones(
                    (*depth.shape, self.n_classes),
                    dtype=depth.dtype,
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_semantics):
        ...
        if hasattr(self._deformation, "get_new_params"):
        ...
        self.enable_grad_weighing(grad_weighing)
        if 'deformation' in optimizable_tensors:
