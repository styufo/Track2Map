	...
	const bool debug)
{
  ...
  torch::Tensor out_semantics = torch::full({NUM_SEM_CHANNELS, H, W}, 0.0, float_opts);
  ...
  return std::make_tuple(rendered, out_color, out_depth, out_alpha, out_semantics, radii, geomBuffer, binningBuffer, imgBuffer);
}
